  ...
  if [ "$elapsed_time_seconds" -gt 900 ]; then
    ...
    if (( $(echo "$unused_cpus >= 3" | bc -l) )); then
      ...
      if [ "$job_check" -eq 3 ]; then
        ...
        body=$(printf "Job ID: %s\nNode: %s\nOwner: %s\nElapsed Time: %sh%sm\nRequested CPU cores: %s\nUnused CPU cores: %s\nSuggested CPU cores: %s\nMetadata: qstat -xf %s\nMonitor: %s\n\nThis is an automated message.\n\nJob %s on Node %s was using an average of %s%% CPU after %s minutes and was stopped $resubmit_result It is suggested to request %s CPU cores (ppn) for future similar jobs instead of %s. Please review both jobs.\n\n\"When requesting fewer resources than a job can consume, the job is allowed to consume as many resources as it can. Thus, the problem for the batch scheduler is requesting more resources than a job would consume and not less, because underperforming jobs will prevent other jobs from being scheduled at the same time\" [1].\n\n\n[1] https://turing.colgate.edu/documentation/pbs_scheduler.php" "$job_id" "$node" "$owner" "$elapsed_hours" "$elapsed_minutes" "$requested_cpus" "$unused_cpus" "$max_cpus" "$job_id" "$monitor_url" "$job_id" "$node" "$qstat_cpu_percentage" "$elapsed_minutes" "$max_cpus" "$requested_cpus")
        {
	  echo "From: \"$supercomuter_name\" <$from_email>"
          echo "To: $email"
          echo "Bcc: $bccemails"
